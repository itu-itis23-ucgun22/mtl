...
| 19 | dinov2_vitb14_reg | 0 **(task-native neck)** | ~90000 | 0.2277 | 0.5951 | 0.7947 | 0.7318 | **🔬 FAZ 3 — DINOv2 task-native neck (Deneme 17)**; donuk DINOv2 (satır 9) ile tek eksen: paylaşılan SFP yerine **det=SFP / seg=ASPP / cls=GAP** (native, ham trunk). Baseline'a ~eşit: **cls +%1.9, det/seg −%1** → **donuk tavanda kazanç yok** (ROADMAP tahmini doğrulandı). Alt-bulgu: donuk DINOv2 seg'inde **çözünürlük > bağlam**. ⚠️ çok-değişkenli (seg decoder da SFP+FCN→ASPP değişti) → saf interference DEĞİL; `per_task_identical` (B) sırada |
